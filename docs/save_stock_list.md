...
```bash
# 전체 종목 동기화 (KOSPI, KOSDAQ만 / ETF 제외)
...
1. **ETF** (시장코드: 8) - 코드만 수집, DB 저장 안함
2. **KOSPI** (시장코드: 0) - ETN/ETF/스팩 제외 후 저장
3. **KOSDAQ** (시장코드: 10) - 스팩 제외 후 저장

ETF를 먼저 처리하는 이유: KOSPI API 응답에 ETF 종목이 포함되어 있어서 중복 방지를 위해 ETF 코드를 먼저 수집 후 KOSPI에서 제외합니다.

**주의:** ETF는 Info 테이블에 저장되지 않습니다. ETF 데이터는 별도 네이버 크롤링으로 수집합니다.
...
### ETF
- 코드 수집만 (KOSPI/KOSDAQ 필터용)
- Info 테이블에 저장하지 않음

## 출력 결과 해석

```
종목목록 저장 시작 (대상: KOSPI, KOSDAQ / ETF 제외)
[ETF] 1049개 코드 수집 (DB 저장 제외)
ETN/ETF/스팩 등 89개 제외
────────────────────────────────────────
[KOSPI] 완료 | API: 960개, 신규: 960개, 업데이트: 0개, 상폐: 0개
ETN/ETF/스팩 등 112개 제외
────────────────────────────────────────
[KOSDAQ] 완료 | API: 1700개, 신규: 1700개, 업데이트: 0개, 상폐: 0개
```
...
```
[KOSPI] 완료 | API: 960개, 신규: 0개, 업데이트: 0개, 상폐: 0개
[KOSDAQ] 완료 | API: 1700개, 신규: 0개, 업데이트: 0개, 상폐: 0개
```
