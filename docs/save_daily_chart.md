...
키움 API에서 일봉 차트 데이터를 조회하여 저장합니다.
...
# 단일 종목, 최근 1일만
python manage.py save_daily_chart --code 005930 --mode last

# 전체 종목, 2년치 (최초 1회)
python manage.py save_daily_chart --code all --mode all

# 전체 종목, 최근 1일만 (정기 업데이트용)
python manage.py save_daily_chart --code all --mode last

# 디버그 모드
python manage.py save_daily_chart --code 005930 --mode all --log-level debug
```
...
| `--mode` | O | `all` (2년 데이터) 또는 `last` (최근 1일만) |
| `--log-level` | X | 로그 레벨 (debug/info/warning/error, 기본: info) |

## 데이터 소스

- API: 키움 Open API (ka10081 - 주식일봉차트조회요청)
...
| 전일대비 | price_change | 전일 대비 가격 변동 |
| 거래량 | trading_volume | 일 거래량 |
| 거래대금 | trading_value | 일 거래대금 |

## 수집 범위

- `--mode all`: 최근 2년치 (연속조회로 전체 수집)
- `--mode last`: 최근 1일만
...
종목코드: 005930 | 모드: all
2년 데이터 조회
저장 완료: 신규 490건, 업데이트 0건

# 전체 종목
일봉 차트 조회 시작 (최근 1일만, 2714개 종목, ETF 제외)
[1/2714] 005930 삼성전자: 신규 0, 업데이트 1
[2/2714] 000660 SK하이닉스: 신규 1, 업데이트 0
[3/2714] 373220 LG에너지솔루션: 데이터 없음
...
======================================================================
일봉 차트 조회 완료: 성공 2700개, 데이터없음 10개, 오류 4개
...
1. 최초 실행: `--mode all`로 2년치 데이터 수집
2. 이후 정기 업데이트: `--mode last`로 최근 1일만 업데이트
