...
'''
    ...
    def handle(self, *args, **options):
        ...
        if options.get('clear'):
            ...
            if code:
                ...
                try:
                    ...
                    deleted_count, _ = InvestorTrend.objects.filter(stock=stock).delete()
                    self.stdout.write(self.style.SUCCESS(f'{stock.name}({code}) InvestorTrend 데이터 {deleted_count}건 삭제 완료'))
        ...
        if code.lower() in ['all', 'fav']:
            ...
            self.log.info(f'투자자 매매동향 저장 시작 (모드: {mode}, 대상: {target_name} {total}개)')
