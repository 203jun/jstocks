...
class Command(BaseCommand):
    help = '기관/외국인 투자동향 조회 및 저장 (종목별투자자기관별요청 - ka10059)'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            choices=['all', 'day'],
            required=True,
            help='조회 모드: all(6개월 데이터), day(최근 거래일 1일만)'
        )
    ...
    def handle(self, *args, **options):
        ...
        if mode == 'day':
            ...
            self.fetch_six_months(token, stock_code)
    ...
    def fetch_six_months(self, token, stock_code):
        ...
        while True:
            ...
            if data_key:
                ...
                if current_batch:
                    dates = [item.get('dt', '') for item in current_batch]
                    oldest = min(dates)
                    newest = max(dates)
                ...
                if current_batch:
                    ...
                    if oldest_date < cutoff_date:
                        self.log.debug(f'6개월 이전 데이터 도달 ({oldest_date}) - 중단')
                        break
