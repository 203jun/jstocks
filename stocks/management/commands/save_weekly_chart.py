...
class Command(BaseCommand):
    help = '주식 주봉 차트 조회 및 저장 (주식주봉차트조회요청 - ka10082)'
    ...
    def process_all_stocks(self, token, mode):
        ...
        mode_desc = '4년 데이터' if mode == 'all' else '최근 1주만'
        self.log.info(f'주봉 차트 조회 시작 ({mode_desc}, {total_count}개 종목, ETF 제외)')
        ...
        # 최종 리포트
        self.log.separator()
        self.log.info(f'주봉 차트 조회 완료: 성공 {success_count}개, 데이터없음 {len(no_data_list)}개, 오류 {len(error_list)}개', success=True)
