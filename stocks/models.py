...
class Info(models.Model):
    ...
    # === 투자지표 ===
    investment_indicator = models.TextField(
        blank=True,
        default='',
        verbose_name='투자지표',
        help_text='투자지표 내용 (마크다운 형식)'
    )
    investment_indicator_updated_at = models.DateField(
        null=True,
        blank=True,
        verbose_name='투자지표 업데이트일',
...
class SectorDiary(models.Model):
    """관심섹터 투자일지"""
...
class MarketIndicator(models.Model):
    ...
    ※ 절대값에는 기준이 없다. +289억이 큰 건지 작은 건지는 시기마다 다르고,
      200일선 +19%도 코스피에선 평상시다(중앙값). 이격도 111 역시 숫자만으로는
      감이 안 오지만 "최근 250일 중 상위 2%"라고 하면 손이 멈춘다.
      그래서 이격도·수급·200일선은 백분위를 함께 저장한다 — "최근 1년의 자기
      자신 대비 어디쯤인가"라 기준이 자동으로 갱신되고, 시장이 바뀌면 임계값도
      따라간다. (ADR 은 이미 그 자체가 비율이라 백분위를 두지 않는다)
    ...
    disparity_pct = models.DecimalField(
        max_digits=5, decimal_places=2,
        null=True, blank=True,
        verbose_name='이격도 백분위',
        help_text='최근 250거래일의 이격도 중 오늘 값의 위치 (0~100)'
    )
